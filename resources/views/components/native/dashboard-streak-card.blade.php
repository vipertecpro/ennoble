...
<native:column class="w-72 gap-4">
    @if ($current === 0)
        <native:row class="items-center gap-4">
            ...
            <native:column class="flex-1 gap-1">
                <native:text class="text-lg font-semibold text-theme-primary-text">No streak yet</native:text>
    ...
    @else
        <native:row class="items-end gap-4">
            <native:column class="flex-1 gap-1">
                <native:text class="text-4xl font-bold text-theme-primary-text">{{ $current }}</native:text>
                <native:text class="text-sm font-semibold text-theme-muted-text">
                    {{ $current === 1 ? 'day in rhythm' : 'days in rhythm' }}
                </native:text>
            </native:column>
            <native:column class="items-end gap-1">
                <native:text class="text-xs font-semibold text-theme-muted-text">LONGEST</native:text>
                <native:text class="text-xl font-bold text-theme-primary-text">{{ $longest }} days</native:text>
            </native:column>
        </native:row>
