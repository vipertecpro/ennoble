            ...
            <native:column class="w-full px-4 pt-4 pb-6 gap-6">
                <native:row class="w-full gap-3">
                    <native:column class="flex-1 items-center gap-1 rounded-2xl bg-theme-surface shadow-sm py-4">
                        <native:text class="text-[18] font-bold text-theme-primary-text">
                            {{ $bestScore === null ? '—' : number_format($bestScore) }}
                        </native:text>
                        <native:text class="text-[11] text-theme-muted-text">Your best</native:text>
                    </native:column>
                    <native:column class="flex-1 items-center gap-1 rounded-2xl bg-theme-surface shadow-sm py-4">
                        <native:text class="text-[18] font-bold text-theme-primary-text">{{ $difficultyLabel }}</native:text>
                        <native:text class="text-[11] text-theme-muted-text">Level</native:text>
                    </native:column>
                </native:row>
                        ...
                        @foreach ($steps as $index => $step)
                            <native:row class="w-full items-start gap-3">
                                <native:column class="w-6 h-6 items-center justify-center rounded-full bg-theme-primary-surface">
                                    <native:text class="text-[12] font-bold text-theme-accent">{{ $index + 1 }}</native:text>
                                </native:column>
                        ...
                        <native:row class="w-full items-center gap-2">
                            @foreach ($skills as $skill)
                                <native:column class="rounded-full bg-theme-secondary-surface px-3 py-1">
                                    <native:text class="text-[12] font-semibold text-theme-secondary-text">{{ $skill }}</native:text>
                                </native:column>
                            @endforeach
                        </native:row>
                    </native:column>
                ...
                @if (count($history) > 0)
                    <native:column class="w-full gap-2">
                        ...
                        <native:column class="w-full rounded-2xl bg-theme-surface shadow-sm">
