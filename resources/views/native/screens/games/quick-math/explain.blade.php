...
<native:column class="h-full w-full {{ Gradients::screen() }} safe-area">
    <native:row class="w-full px-4 pt-3 pb-2 items-center gap-3">
        <native:pressable
            ...
            :press-scale="0.9"
            class="w-9 h-9 items-center justify-center rounded-full bg-theme-surface border border-rose-500/15"
        >
    ...
    <native:scroll-view class="flex-1 w-full" scroll-anchor="bottom">
        <native:column class="w-full px-4 py-4 gap-3">
            @foreach ($visibleSteps as $index => $step)
                ...
                >
                    <native:column class="w-8 h-8 items-center justify-center rounded-full bg-linear-to-br from-rose-500 to-orange-400 shadow-sm">
                        <x-native.ui.icon :ios="Ios::Sparkles" :android="Android::AutoAwesome" :size="15" color="#000000" dark-color="#000000" />
                    </native:column>

                    <native:column class="rounded-2xl bg-theme-surface border border-rose-500/15 shadow-md px-4 py-3 max-w-[80%]">
            ...
            @if ($typing)
                <native:row class="w-full items-end gap-2" a11y-label="Assistant is typing">
                    ...
                    <native:row class="items-center rounded-2xl bg-theme-surface border border-rose-500/15 shadow-md px-4 py-4 gap-2">
                        @for ($dot = 0; $dot < 3; $dot++)
                            <native:circle
                                :width="7"
                                :height="7"
